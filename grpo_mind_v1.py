    ...
    while rem_a or rem_b:
        ...
        for sig_round in range(max_signals):
            ...
            if rem_a:
                ...
                sig_turns_a.append(MindTurn(p_a, raw_a,
                                            -JUDGE_PENALTY if jv_a else 0.0,
                                            judge_violation=jv_a))
            ...
            if rem_b:
                ...
                sig_turns_b.append(MindTurn(p_b, raw_b,
                                            -JUDGE_PENALTY if jv_b else 0.0,
                                            judge_violation=jv_b))
        ...
        for ri, rnd in enumerate(ep.rounds):
            ...
            for si in range(n_sig):
                if si < len(rnd.sig_turns_a):
                    ...
                    f.write(f"    Signal {si+1} A: \"{t.completion[:80]}\"{jv}\n")
                if si < len(rnd.sig_turns_b):
                    t = rnd.sig_turns_b[si]
                    jv = " [JUDGE]" if t.judge_violation else ""
                    f.write(f"    Signal {si+1} B: \"{t.completion[:80]}\"{jv}\n")
    ...
    for ri, rnd in enumerate(ep.rounds):
        ...
        for si in range(max(len(rnd.sig_turns_a), len(rnd.sig_turns_b))):
            if si < len(rnd.sig_turns_a):
                t = rnd.sig_turns_a[si]
                jv = " [JUDGE]" if t.judge_violation else ""
                print(f"    Sig {si+1} A: \"{t.completion[:60]}\"{jv}")
            if si < len(rnd.sig_turns_b):
                t = rnd.sig_turns_b[si]
                jv = " [JUDGE]" if t.judge_violation else ""
                print(f"    Sig {si+1} B: \"{t.completion[:60]}\"{jv}")
